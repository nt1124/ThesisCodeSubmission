struct builderInputCommitStruct *initBuilderInputCommitStruct(int numCircuits, int numInputs, gmp_randstate_t *state)
{
	...
	outputStruct -> c_boxes_0 = (struct Peder_Hash_commit_box ***) calloc(numCircuits, sizeof(struct Peder_Hash_commit_box **));
	outputStruct -> c_boxes_1 = (struct Peder_Hash_commit_box ***) calloc(numCircuits, sizeof(struct Peder_Hash_commit_box **));
	outputStruct -> k_boxes_0 = (struct Peder_Hash_commit_key ***) calloc(numCircuits, sizeof(struct Peder_Hash_commit_key **));
	outputStruct -> k_boxes_1 = (struct Peder_Hash_commit_key ***) calloc(numCircuits, sizeof(struct Peder_Hash_commit_key **));
	...
	for(i = 0; i < numCircuits; i ++)
	{
		...
		outputStruct -> c_boxes_0[i] = (struct Peder_Hash_commit_box **) calloc(numInputs, sizeof(struct Peder_Hash_commit_box *));
		outputStruct -> c_boxes_1[i] = (struct Peder_Hash_commit_box **) calloc(numInputs, sizeof(struct Peder_Hash_commit_box *));
		outputStruct -> k_boxes_0[i] = (struct Peder_Hash_commit_key **) calloc(numInputs, sizeof(struct Peder_Hash_commit_key *));
		outputStruct -> k_boxes_1[i] = (struct Peder_Hash_commit_key **) calloc(numInputs, sizeof(struct Peder_Hash_commit_key *));
...
struct builderInputCommitStruct *deserialiseC_Boxes(unsigned char *inputBuffer, gmp_randstate_t *state, int *inputOffset)
{
	...
	outputStruct -> c_boxes_0 = (struct Peder_Hash_commit_box ***) calloc(outputStruct -> numCircuits, sizeof(struct Peder_Hash_commit_box **));
	outputStruct -> c_boxes_1 = (struct Peder_Hash_commit_box ***) calloc(outputStruct -> numCircuits, sizeof(struct Peder_Hash_commit_box **));
	...
	for(i = 0; i < numCircuits; i ++)
	{
		if(0x01 == jSetOwn[i])
		{
			...
			for(j = 0; j < numInputs; j ++)
			{
				...
				{
					validCommitments0 |= single_decommit_raw_Peder_Hash_R(commitStruct -> params, commitStruct -> c_boxes_0[i][j],
																	x0, tempR0List[i][j], 16);
					validCommitments0 |= single_decommit_raw_Peder_Hash_R(commitStruct -> params, commitStruct -> c_boxes_1[i][j],
	...
	for(i = 0; i < numCircuits; i ++)
	{
		if(0x00 == jSetOwn[i])
		{
			for(j = 0; j < numInputs; j ++)
			{
				...
				{
					validCommitments1 |= single_decommit_raw_Peder_Hash_R(commitStruct -> params, commitStruct -> c_boxes_0[i][j],
																	x0, tempR_List[i][j], 16);
				}
				else
				{
					validCommitments1 |= single_decommit_raw_Peder_Hash_R(commitStruct -> params, commitStruct -> c_boxes_1[i][j],
																	x0, tempR_List[i][j], 16);
				}
			}
		}
