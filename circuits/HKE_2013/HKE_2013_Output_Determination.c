...
struct secureEqualityCommitments *commitForEqualityTest(unsigned char **secureEqualityInputs, gmp_randstate_t *state, int numOutputs)
{
	...
	outputStruct -> c_boxes_0 = (struct Peder_Hash_commit_box **) calloc(numOutputs, sizeof(struct Peder_Hash_commit_box *));
	outputStruct -> c_boxes_1 = (struct Peder_Hash_commit_box **) calloc(numOutputs, sizeof(struct Peder_Hash_commit_box *));
	outputStruct -> k_boxes_0 = (struct Peder_Hash_commit_key **) calloc(numOutputs, sizeof(struct Peder_Hash_commit_key *));
	outputStruct -> k_boxes_1 = (struct Peder_Hash_commit_key **) calloc(numOutputs, sizeof(struct Peder_Hash_commit_key *));
									...
									int numOutputs, int *bufferOffset)
{
	...
	for(i = 0; i < numOutputs; i ++)
	{
		...
		test0 = single_decommit_raw_Peder_Hash_R(commitStruct_Partner -> params, commitStruct_Partner -> c_boxes_0[i], inputBuffer + tempOffset, *tempMPZ, xLen0);
		tempOffset += xLen0;
		...
		test1 = single_decommit_raw_Peder_Hash_R(commitStruct_Partner -> params, commitStruct_Partner -> c_boxes_1[i], inputBuffer + tempOffset, *tempMPZ, xLen1);
		tempOffset += xLen1;
