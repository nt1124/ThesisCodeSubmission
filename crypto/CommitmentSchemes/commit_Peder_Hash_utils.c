...
int sizeOf_C_Box_Serial(struct Peder_Hash_commit_box *c)
{
	int totalLength = 0;
...
int sizeOf_K_Box_Serial(struct Peder_Hash_commit_key *k, int msgLength)
{
...
unsigned char *serialiseAllC_Boxes_1D(struct Peder_Hash_commit_box **c, int numC_Boxes, int *totalLength)
{
	...
	for(i = 0 ; i < numC_Boxes; i ++)
	{
		serialise_Peder_Hash_Cbox(c[i], outputBuffer, &tempOffset);
	}
...
struct Peder_Hash_commit_box **deserialiseAllC_Boxes_1D(unsigned char *inputBuffer, int numC_Boxes, int *bufferOffset)
{
	struct Peder_Hash_commit_box **c = (struct Peder_Hash_commit_box **) calloc(numC_Boxes, sizeof(struct Peder_Hash_commit_box *));
	int outputLength, i, tempOffset = *bufferOffset;


	for(i = 0 ; i < numC_Boxes; i ++)
	{
		c[i] = deserialise_Peder_Hash_Cbox(inputBuffer, &tempOffset);
	}
...
unsigned char *serialiseAllC_Boxes_2D(struct Peder_Hash_commit_box ***c, int iMax, int jMax, int *totalLength)
{
	unsigned char *outputBuffer;
	...
	for(i = 0; i < iMax; i ++)
	{
		for(j = 0 ; j < jMax; j ++)
		{
			serialise_Peder_Hash_Cbox(c[i][j], outputBuffer, &tempOffset);
		}
	}
...
struct Peder_Hash_commit_box ***deserialiseAllC_Boxes_2D(unsigned char *inputBuffer, int iMax, int jMax, int *bufferOffset)
{
	struct Peder_Hash_commit_box ***c = (struct Peder_Hash_commit_box ***) calloc(iMax, sizeof(struct Peder_Hash_commit_box **));
	int outputLength, i, j, tempOffset = *bufferOffset;


	for(i = 0 ; i < iMax; i ++)
	{
		c[i] = (struct Peder_Hash_commit_box **) calloc(jMax, sizeof(struct Peder_Hash_commit_box *));
		for(j = 0 ; j < jMax; j ++)
		{
			c[i][j] = deserialise_Peder_Hash_Cbox(inputBuffer, &tempOffset);
		}
	}
...
unsigned char *serialiseAllK_Boxes_1D(struct Peder_Hash_commit_key **k, int msgLength, int numK_Boxes, int *totalLength)
{
	unsigned char *outputBuffer;
	int outputLength, i, tempOffset = 0;
	...
	for(i = 0 ; i < numK_Boxes; i ++)
	{
		serialise_Peder_Hash_Kbox(k[i], msgLength, outputBuffer, &tempOffset);
	}
...
struct Peder_Hash_commit_key **deserialiseAllK_Boxes_1D(unsigned char *inputBuffer, int msgLength, int numK_Boxes, int *bufferOffset)
{
	struct Peder_Hash_commit_key **k = (struct Peder_Hash_commit_key **) calloc(numK_Boxes, sizeof(struct Peder_Hash_commit_key *));
	int outputLength, i, tempOffset = *bufferOffset;


	for(i = 0 ; i < numK_Boxes; i ++)
	{
		k[i] = deserialise_Peder_Hash_Kbox(inputBuffer, msgLength, &tempOffset);
	}
