...
randctx *getRandCtxByCoinToss(int writeSocket, int readSocket, randctx *ownCTX, gmp_randstate_t *state, int partyID)
{
	...
	struct Peder_Hash_commit_box *ownC_Box, *partnerC_Box;
	struct Peder_Hash_commit_key *ownK_Box, *partnerK_Box;
	...
	serialise_Peder_Hash_Cbox(ownC_Box, commBufferWrite, &bufferOffset);

	commBufferRead = sendReceiveExchange(writeSocket, readSocket, commBufferWrite,
										commBufferWriteLen, &commBufferReadLen, partyID);
	bufferOffset = 0;
	partnerC_Box = deserialise_Peder_Hash_Cbox(commBufferRead, &bufferOffset);
	free(commBufferWrite);
	free(commBufferRead);
	...
	commBufferWrite = (unsigned char *) calloc(commBufferWriteLen, sizeof(unsigned char));
	serialise_Peder_Hash_Kbox(ownK_Box, commitLen, commBufferWrite, &bufferOffset);

	bufferOffset = 0;
	commBufferRead = sendReceiveExchange(writeSocket, readSocket, commBufferWrite,
										commBufferWriteLen, &commBufferReadLen, partyID);
	partnerK_Box = deserialise_Peder_Hash_Kbox(commBufferRead, commitLen, &bufferOffset);

	// Decommit to get the partners seed
	bufferOffset = 0;
	partnerSeedBytes = single_decommit_Peder_Hash_R(partnerParams, partnerC_Box, partnerK_Box, commitLen, &bufferOffset);
	free(commBufferWrite);
	free(commBufferRead);
